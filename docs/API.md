...
**Field notes:**
- `schedule.mode` — `"scheduled"` only when **nothing** claims 24/7 AND real `begin_time`/`end_time` values are present. Two independent fields claim it and neither implies the other (Quirk 21): `onTimeSwitch` non-zero (the "Continuous 24H/7D" app toggle) **or** `switchTime` bit 7 (128) on the automation's first rule. An **unrecognised** `onTimeSwitch` value fails safe to `"continuous"` — a schedule the device may not be keeping is worse than none. Sentinel times (255 / 65535) also yield `"continuous"`, since no window is readable. Reading only `onTimeSwitch` here is what let one response call the same automation both continuous and scheduled (#329); `rules[].window` and `human_summary` now derive from the same reconciled value.
